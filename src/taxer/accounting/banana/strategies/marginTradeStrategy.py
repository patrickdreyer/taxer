...
class MarginTradeStrategy(BananaStrategy):
    ...
    def transform(self, transaction):
        ...
        if e.amount > 0:
            yield self._book(transaction, ['Margin Trade - Einstieg', self.__accounts.fees,   e.account,              e.amount, e.unit,   e.baseCurrency.exchangeRate, e.baseCurrency.amount, '',    e.costCenter.minus()])
        if a.amountRaw >= 0:
            MarginTradeStrategy.__log.debug("%s - Margin gain; %s, %s", transaction.dateTime, transaction.mergentId, a)
            yield self._book(transaction, ['Margin Trade - Gewinn',   a.account,              self.__accounts.equity, a.amount, a.unit,   a.baseCurrency.exchangeRate, a.baseCurrency.amount, '',    a.costCenter])
        else:
            MarginTradeStrategy.__log.debug("%s - Margin loss; %s, %s", transaction.dateTime, transaction.mergentId, a)
            yield self._book(transaction, ['Margin Trade - Verlust',  self.__accounts.equity, a.account,              a.amount, a.unit,   a.baseCurrency.exchangeRate, a.baseCurrency.amount, '',    a.costCenter.minus()])
        yield self._book(transaction,     ['Margin Trade - Ausstieg', self.__accounts.fees,   a.account,              x.amount, a.unit,   x.baseCurrency.exchangeRate, x.baseCurrency.amount, '',    x.costCenter.minus()])
