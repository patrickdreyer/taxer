...
class PaymentStrategy(BananaStrategy):
    ...
    def transform(self, transaction):
        ...
        if w.amount > 0:
            ...
            yield self._book(transaction, [description, self.__accounts.equity, w.account,  w.amount, w.unit,   w.baseCurrency.exchangeRate, w.baseCurrency.amount, '',     w.costCenter.minus(), transaction.note])
        f = self._currency(transaction.fee, transaction)
        if f.amount > 0:
            # fee
            yield self._book(transaction, [description, self.__accounts.fees,   w.account,  f.amount, f.unit,   f.baseCurrency.exchangeRate, f.baseCurrency.amount, '',     f.costCenter.minus()])
